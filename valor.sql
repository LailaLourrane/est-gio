...
INSERT INTO Faturamento (Dia, Valor)
SELECT 
    T.N.value('(dia)[1]', 'INT') AS Dia,
    T.N.value('(valor)[1]', 'DECIMAL(18,4)') AS Valor
FROM 
    @xml.nodes('/rows/row') AS T(N);

WITH FaturamentoCalculo AS (
    SELECT
        Valor,
        MIN(Valor) OVER () AS MenorFaturamento,
        MAX(Valor) OVER () AS MaiorFaturamento,
        AVG(CASE WHEN Valor > 0 THEN Valor END) OVER () AS MediaFaturamento
    FROM
        Faturamento
    WHERE
        Valor > 0
...
SELECT
    DISTINCT MenorFaturamento,
    MaiorFaturamento,
    (SELECT COUNT(*) FROM FaturamentoCalculo WHERE Valor > MediaFaturamento) AS DiasAcimaDaMedia
FROM
    FaturamentoCalculo;
